...
    { key: PERMISSIONS.PRODUCTS_CREATE, label: 'Create Product', group: 'Products' },
    { key: PERMISSIONS.PRODUCTS_EDIT, label: 'Edit Product', group: 'Products' },
    { key: PERMISSIONS.PRODUCTS_DELETE, label: 'Delete Product', group: 'Products' },
    
    // Categories
    { key: PERMISSIONS.CATEGORIES_VIEW, label: 'View Categories', group: 'Categories' },
    { key: PERMISSIONS.CATEGORIES_CREATE, label: 'Create Category', group: 'Categories' },
    { key: PERMISSIONS.CATEGORIES_EDIT, label: 'Edit Category', group: 'Categories' },
    { key: PERMISSIONS.CATEGORIES_DELETE, label: 'Delete Category', group: 'Categories' },
    
    // Orders
    { key: PERMISSIONS.ORDERS_VIEW, label: 'View Orders', group: 'Orders' },
    { key: PERMISSIONS.ORDERS_EDIT, label: 'Edit Order', group: 'Orders' },
    { key: PERMISSIONS.ORDERS_DELETE, label: 'Delete Order', group: 'Orders' },
    
    // Users
    { key: PERMISSIONS.USERS_VIEW, label: 'View Users', group: 'Users' },
    { key: PERMISSIONS.USERS_EDIT, label: 'Edit User', group: 'Users' },
    { key: PERMISSIONS.USERS_DELETE, label: 'Delete User', group: 'Users' },
    
    // Accounts
    { key: PERMISSIONS.ACCOUNTS_VIEW, label: 'View Accounts', group: 'Accounts' },
    { key: PERMISSIONS.ACCOUNTS_CREATE, label: 'Create Account', group: 'Accounts' },
    { key: PERMISSIONS.ACCOUNTS_EDIT, label: 'Edit Account', group: 'Accounts' },
    { key: PERMISSIONS.ACCOUNTS_DELETE, label: 'Delete Account', group: 'Accounts' },
    
    // Roles
...
    { key: PERMISSIONS.ROLES_PERMISSIONS, label: 'Manage Permissions', group: 'Roles' },
    
    // Settings
...
  // KHẮC PHỤC LỖI: Sử dụng p.id (getter string) thay vì p._id để đảm bảo trích xuất ID hợp lệ
...
  } catch (error) {
    if (error.code === 11000) {
      console.warn('⚠️ Root Admin account already exists (Duplicate key error).');
...
    
    // Đảm bảo luôn có account để gán createdBy
    const createdByAccount = accounts.length > 0 ? accounts[Math.floor(Math.random() * accounts.length)] : null;
    
    products.push({
      title: title,
      slug: generateSlug(title),
...
    // 2. Seed other Users and Accounts
    const users = await seedUsers(10);
    // Ghép Root Account và các tài khoản ngẫu nhiên khác. 
    const accounts = [rootAccount, ...(await seedAccounts(roles, 5))].filter(Boolean); 
